...
由 [Exa](https://exa.ai) 支持的 `WebSearchProvider`，用于 harness [web 能力 seam](../web/README.zh.md)（`ctx.web`）。配置 API Key 时调用 Exa 的 `POST /search`；未配置时使用 Exa 的免凭据托管 MCP。两条路径都会映射为 seam 规范化的 `WebSearchResult`。
...
| 配置键 | 默认值 | 含义 |
|---|---|---|
| `apiKey` | `$EXA_API_KEY` | 可选 Exa API 密钥。为空或缺失时使用匿名托管 MCP。 |
| `baseURL` | `https://api.exa.ai` | 端点基址；追加 `/search`。无法解析时提供方不可用。 |
| `mcpURL` | `https://mcp.exa.ai/mcp` | 未配置 API Key 时使用的托管 MCP 端点。 |
...
带 Key 的 REST 路径映射 Exa 的扁平 `results[]`；匿名路径映射托管 MCP 返回的 `Title`／`URL`／`Published`／`Highlights` 分段。两者都生成 `WebSearchSource`，并省略生成式 `content`。请求的 `maxResults` 优先于已配置的默认 `numResults` 并发送给 Exa；最终上限由 seam 强制执行。提供方失败以 `WebError` `WEB_PROVIDER_ERROR` 呈现，中止请求以 `WEB_ABORTED` 呈现。HTTP 重定向会在访问 `Location` 目标前被拒绝。
...
- **匿名托管 MCP 有速率限制**：需要更高额度时设置 `EXA_API_KEY`，provider 会自动切换到带 Key 的 REST 路径。
